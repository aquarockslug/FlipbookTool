...
class Cropper(App):
    ...
    def createCrop(self, selection: list):
        ...
        # find distance between points and the middle of the line
        distance = ((x1 - x2)**2 + (y1 - y2)**2)**0.5

        center = (x2, y2)# TODO: find point between selection points

        self.crop = self.ic.curr.crop(center, distance / 100)
        self.crop.show()
    ...
    def incrementScale(self, d):
        self.ic.cropScale = self.ic.cropScale + 1
        d.text = str(self.ic.cropScale)

    def deincrementScale(self, d):
        self.ic.cropScale = self.ic.cropScale - 1 if self.ic.cropScale > 1 else 1
        d.text = str(self.ic.cropScale)
    ...
    def build(self):
        ...
        cropScaleDisplay = Label(text='3', pos=(
            75, self.size[1]-75), font_size=24, bold=True)
